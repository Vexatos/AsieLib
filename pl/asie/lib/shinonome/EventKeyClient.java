...
public class EventKeyClient {
	...
	public void asieEvent(RenderPlayerEvent.SetArmorModel event) {
		if(EventKeyServer.isEntityNano(event.entityPlayer) && event.slot == 1 && (event.stack == null || event.stack.getItem() == AsieLibMod.itemKey)) {
			...
			try {
				RenderPlayer.class.getMethod("bindTexture", ResourceLocation.class).invoke(event.renderer, RenderBiped.getArmorResource(event.entityPlayer, key, 1, null));
	            ModelBiped modelbiped = (ModelBiped)RenderPlayer.class.getField("modelArmorChestplate").get(event.renderer);
	            ...
	            ModelBiped mainModel = (ModelBiped)RenderPlayer.class.getField("mainModel").get(event.renderer);
